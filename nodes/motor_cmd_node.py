...
def command_callback(msg):
    ...
    if command == 0:
        ...
        motor_ctrl.move_forward(1000, 50)  # steps=1000, rpm=50
    elif command == 1:
        # Move backward
        rospy.loginfo("Moving backward ~3s.")
        motor_ctrl.move_backward(1000, 50)
    elif command == 2:
        # Turn left
        rospy.loginfo("Turning left ~3s. (left off, right forward)")
        # left off => rotate(0), right => forward 1000 steps
        motor_ctrl.motor_left.rotate(0, 50)   # do nothing
        motor_ctrl.motor_right.rotate(1000, 50)
    elif command == 3:
        # Turn right
        rospy.loginfo("Turning right ~3s. (right off, left forward)")
        motor_ctrl.motor_left.rotate(1000, 50)
        motor_ctrl.motor_right.rotate(0, 50)
    elif command == 4:
        # Spin left

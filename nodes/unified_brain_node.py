...
class UnifiedBrainNode:
    def __init__(self):
        ...
        self.last_motor_cmd   = None  # track last command to avoid duplicates

        # Timer for forward keep-alive
    ...
    def send_cmd(self, cmd):
        """Publish cmd to /pollux/motor_cmd if it's different from last_motor_cmd."""
        if cmd != self.last_motor_cmd:
            self.cmd_pub.publish(Int32(data=cmd))
            self.last_motor_cmd = cmd

    # --------------------------------------------------------------------------
    # Periodic forward "keep alive"
    # --------------------------------------------------------------------------
    def forward_timer_cb(self, event):
        # Only issue FORWARD_CMD if:
        #  1) Not in an avoidance action
        #  2) Last cmd was not already FORWARD_CMD
    ...
    def bottom_sensors_callback(self, msg):
        ...
        self.in_action = False
        # Only now are we free to do forward (but let's do it once)
        self.send_cmd(FORWARD_CMD)
    ...
    def front_sensors_callback(self, msg):
        ...
        # Both sensors => full 180 + random spin
